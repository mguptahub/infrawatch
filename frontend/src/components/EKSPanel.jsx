...
export default function EKSPanel() {
  ...
  return (
    <section className="panel">
      <AlertBanner serviceType="eks" onAlarmsLoaded={setResourceAlarms} />
          ...
          <table className="data-table">
            ...
            <tbody>
              {sorted.map(c => (
                ...
                >
                  <td>
                    {resourceAlarms.some(a => a.resource_id === c.name && a.state === "ALARM") && (
                      <span className="alert-dot" title={`${resourceAlarms.filter(a => a.resource_id === c.name && a.state === "ALARM").length} active alarm(s)`} />
                    )}
                  </td>
...
function DetailDrawer({ cluster, onClose }) {
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (showClusterMetrics) {
        setShowClusterMetrics(false);
        return;
      }
      if (showNodes) return;
      onClose();
  ...
  }, [onClose, showNodes, showClusterMetrics]);
  ...
  return (
    ...
    <div className="detail-drawer">
      ...
      <div className="drawer-body">
        ...
        {detail  && (
          <>
            <ResourceAlerts resourceId={cluster.name} />
            {/* ── Overview ── */}
      ...
      {showClusterMetrics && (
        <ClusterMetricsModal cluster={cluster} onClose={() => setShowClusterMetrics(false)} />
      )}
    </>
  );
}

function ClusterMetricsModal({ cluster, onClose }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    api.getEKSMetrics(cluster.name, hours, cluster.region)
      .then(d => { setData(d); setLoading(false); })
      .catch(e => { setError(e.message); setLoading(false); });
  }, [cluster.name, cluster.region, hours]);

  const metricConfigs = [
    { key: "apiserver_request_total", title: "API Server Requests (total)", color: "var(--blue)", yFmt: (v) => v },
    { key: "apiserver_request_total_4XX", title: "API Server 4XX", color: "var(--amber)", yFmt: (v) => v },
    { key: "apiserver_request_total_5XX", title: "API Server 5XX", color: "var(--red)", yFmt: (v) => v },
    { key: "scheduler_pending_pods", title: "Scheduler Pending Pods", color: "var(--teal)", yFmt: (v) => v },
    { key: "scheduler_schedule_attempts_total", title: "Scheduler Schedule Attempts", color: "var(--green)", yFmt: (v) => v },
    { key: "apiserver_storage_size_bytes", title: "etcd Storage (bytes)", color: "var(--brand)", yFmt: (v) => (v / 1e6).toFixed(1) + " MB" },
  ];
      ...
      <div className="metrics-body">
        ...
        {data && (
          <div className="metrics-charts-grid">
            {metricConfigs.map(({ key, title, color, yFmt }) => (
              <EksMetricChart
                key={key}
                title={title}
                hours={hours}
...
function EksMetricChart({ title, hours, data, color, yFmt, emptyNote }) {
  ...
  const tickInterval = isEmpty ? 0 : Math.max(0, Math.floor((data?.length || 0) / 6) - 1);
  return (
    <div className="metrics-chart-card">
      <div className="metrics-chart-title">{title}</div>
      {isEmpty ? (
        <div className="metrics-chart-empty">{emptyNote}</div>
      ) : (
        <ResponsiveContainer width="100%" height={180}>
