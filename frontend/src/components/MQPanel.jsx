...
import { RefreshCw, X, Info, Layers, BarChart2, ChevronDown, ChevronRight, Check } from "lucide-react";
import { 
...
export default function MQPanel() {
  ...
  async function handleRefresh() {
    ...
    const streamUrl = api.mqRefreshStreamUrl();
  ...
  return (
    <section className="panel">
      <AlertBanner serviceType="mq" onAlarmsLoaded={setResourceAlarms} />
          ...
          <table className="data-table">
            ...
            <tbody>
              {sorted.map((b) => (
                ...
                >
                  <td>
                    {resourceAlarms.some(a => a.resource_id === b.id && a.state === "ALARM") && (
                      <span className="alert-dot" title={`${resourceAlarms.filter(a => a.resource_id === b.id && a.state === "ALARM").length} active alarm(s)`} />
                    )}
                  </td>
  ...
  return (
    <div className="detail-drawer">
      ...
      <div className="drawer-body">
        ...
        {detail && (
          <>
            <ResourceAlerts resourceId={broker.id} />
