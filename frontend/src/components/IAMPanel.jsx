    ...
    <>
      ...
      <div className="detail-drawer">
        ...
        <div className="drawer-body">
          ...
          {detail && (
            <>
              <ResourceAlerts resourceId={user.username} />
...
export default function IAMPanel() {
  ...
  async function handleRefresh() {
    ...
    const streamUrl = api.iamRefreshStreamUrl();
  ...
  return (
    <section className="panel">
      <AlertBanner serviceType="iam" onAlarmsLoaded={setResourceAlarms} />
            ...
            <tbody>
              {sorted.map((u) => (
                <tr key={u.user_id} className="row-clickable" onClick={() => setSelected(u)}>
                  <td>
                    {resourceAlarms.some(a => a.resource_id === u.username && a.state === "ALARM") && (
                      <span className="alert-dot" title={`${resourceAlarms.filter(a => a.resource_id === u.username && a.state === "ALARM").length} active alarm(s)`} />
                    )}
                  </td>
