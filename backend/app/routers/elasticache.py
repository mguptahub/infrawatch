...
def _metrics_elasticache_from_db(resource_id: str, region: str, hours: int, db: Session):
    start = datetime.utcnow() - timedelta(hours=hours)
    rows = (
        db.query(CollectedMetric)
        .filter(
            CollectedMetric.service_type == "elasticache",
    ...
    results = {}
    for m in rows:
        results.setdefault(m.metric_name, []).append({"time": m.timestamp.isoformat() + "Z", "value": round(m.value, 2)})
    for key in ["CPUUtilization", "CurrConnections", "DatabaseMemoryUsagePercentage", "CacheHits", "CacheMisses"]:
        results.setdefault(key, [])
        results[key].sort(key=lambda x: x["time"])
    return results


@router.get("/metrics")
def get_cluster_metrics(request: Request, id: str, engine: str = "redis", hours: int = 1, is_rg: bool = True, db: Session = Depends(get_db)):
    _, config = get_session_and_config(request)
    if USE_COLLECTOR_DB:
        return _metrics_elasticache_from_db(id, config.region, hours, db)
    session, _ = get_session_and_config(request)
    cw = session.client("cloudwatch")
