...
def _metrics_ec2_from_db(instance_id: str, region: str, hours: int, db: Session):
    ...
    by_metric = {}
    for m in rows:
        by_metric.setdefault(m.metric_name, []).append({"ts": m.timestamp.isoformat() + "Z", "v": round(m.value, 3)})

    def _to_list(name):
        return [x for x in sorted(by_metric.get(name, []), key=lambda p: p["ts"])]

    # Merge Disk* and EBS* for disk_read / disk_write (same as live fetch)
    disk_read = {}
    for p in by_metric.get("DiskReadBytes", []) + by_metric.get("EBSReadBytes", []):
        disk_read[p["ts"]] = disk_read.get(p["ts"], 0) + p["v"]
    disk_write = {}
    for p in by_metric.get("DiskWriteBytes", []) + by_metric.get("EBSWriteBytes", []):
        disk_write[p["ts"]] = disk_write.get(p["ts"], 0) + p["v"]
    ...
    return {
        ...
        "metrics": {
            "cpu": _to_list("CPUUtilization"),
            "network_in": _to_list("NetworkIn"),
            "network_out": _to_list("NetworkOut"),
            "disk_read": [{"ts": k, "v": round(v, 3)} for k, v in sorted(disk_read.items())],
            "disk_write": [{"ts": k, "v": round(v, 3)} for k, v in sorted(disk_write.items())],
            "memory": [],  # requires CWAgent on instance
        },
    }
