...
def _metrics_docdb_from_db(resource_id: str, region: str, hours: int, db: Session):
    start = datetime.utcnow() - timedelta(hours=hours)
    rows = (
        db.query(CollectedMetric)
        .filter(
            CollectedMetric.service_type == "docdb",
    ...
    for key in ["CPUUtilization", "DatabaseConnections", "FreeStorageSpace", "ReadIOPS", "WriteIOPS"]:
        out.setdefault(key, [])
...
@router.get("/metrics")
def get_docdb_metrics(request: Request, id: str, is_cluster: bool = False, hours: int = 24, db: Session = Depends(get_db)):
    _, config = get_session_and_config(request)
    if USE_COLLECTOR_DB:
        return _metrics_docdb_from_db(id, config.region, hours, db)
    session, _ = get_session_and_config(request)
