...
def _metrics_eks_from_db(cluster_name: str, region: str, hours: int, db: Session):
    """Return EKS control-plane metrics time-series from collected_metrics."""
    start = datetime.utcnow() - timedelta(hours=hours)
    rows = (
        db.query(CollectedMetric)
        .filter(
            CollectedMetric.service_type == "eks",
            CollectedMetric.resource_id == cluster_name,
            CollectedMetric.region == region,
...
@router.get("/clusters/{name}/metrics")
def get_eks_cluster_metrics(
    name: str,
    request: Request,
    hours: int = 24,
    ...
    use_region = region or config.region
    if USE_COLLECTOR_DB:
        return _metrics_eks_from_db(name, use_region, hours, db)
    raise HTTPException(status_code=501, detail="EKS metrics from live CloudWatch not implemented; use collector DB.")
