...
def _metrics_opensearch_from_db(domain_name: str, region: str, hours: int, db: Session):
    start = datetime.utcnow() - timedelta(hours=hours)
    rows = (
        db.query(CollectedMetric)
        .filter(
            CollectedMetric.service_type == "opensearch",
            CollectedMetric.resource_id == domain_name,
    ...
    out = {}
    for m in rows:
        out.setdefault(m.metric_name, []).append({"time": m.timestamp.isoformat() + "Z", "value": round(m.value, 4)})
    for key in ["CPUUtilization", "JVMMemoryPressure", "FreeStorageSpace", "SearchRate", "IndexingRate", "SysMemoryUtilization"]:
        out.setdefault(key, [])
...
@router.get("/metrics")
def get_opensearch_metrics(request: Request, name: str, hours: int = 24, db: Session = Depends(get_db)):
    _, config = get_session_and_config(request)
    if USE_COLLECTOR_DB:
        return _metrics_opensearch_from_db(name, config.region, hours, db)
    session, _ = get_session_and_config(request)
    os_client = session.client("opensearch")
    cw = session.client("cloudwatch")
