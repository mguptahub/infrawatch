...
def _metrics_mq_from_db(broker_id: str, region: str, hours: int, db: Session):
    start = datetime.utcnow() - timedelta(hours=hours)
    rows = (
        db.query(CollectedMetric)
        .filter(
            CollectedMetric.service_type == "mq",
            CollectedMetric.resource_id == broker_id,
    ...
    out = {}
    for m in rows:
        out.setdefault(m.metric_name, {})[m.timestamp.isoformat() + "Z"] = m.value
    # ActiveMQ: use HeapUsage as memory if MemoryUsage not present
    if not out.get("memory") and out.get("memory_heap"):
        out["memory"] = out["memory_heap"]
    return out


@router.get("/brokers/{broker_id}/metrics")
def get_mq_metrics(broker_id: str, request: Request, hours: int = 24, db: Session = Depends(get_db)):
    _, config = get_session_and_config(request)
    if USE_COLLECTOR_DB:
        return _metrics_mq_from_db(broker_id, config.region, hours, db)
    session, _ = get_session_and_config(request)
    try:
